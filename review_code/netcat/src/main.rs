...
use server::*;
mod server {
    // use std::{fs::write, io::{BufRead, BufReader, BufWriter, Write}, mem::Discriminant, os::windows::process, thread};
    ...
    pub fn start_server(socket_address: String) -> IOResult<()> {
        ...
        loop {
            ...
            thread::spawn(|| match handle_connection(tcp_stream) {
                ...
                Err(_) => {
                    log::info!("Problem with client connection...");
                }
            });
        }
    }
